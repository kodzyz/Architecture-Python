...
class UnitOfWork:
    ...
    current = threading.local()  # операции с БД выполняются в одном потоке

    def __init__(self):
        '''три списка с объктами моделей'''
        self.new_objects = []
        self.dirty_objects = []
...
try:
    UnitOfWork.new_current()  # когда начинаем операции с БД нужно инитиал новый поток
    new_person_1 = Person(None, 'Igor', 'Igorev')
    new_person_1.mark_new()  # отмечаем для вставки в БД => см.DomainObject

    new_person_2 = Person(None, 'Fedor', 'Fedorov')
    new_person_2.mark_new()  # отмечаем для вставки в БД

    person_mapper = PersonMapper(connection)
    exists_person_1 = person_mapper.find_by_id(1)
    exists_person_1.mark_dirty()  # помечаем запись для изменения
    print(exists_person_1.first_name)
    exists_person_1.first_name += ' Senior'
    print(exists_person_1.first_name)

    exists_person_2 = person_mapper.find_by_id(2)
    exists_person_2.mark_removed()  # помечаем запись для удаления

    print(UnitOfWork.get_current().__dict__)

    # запуск всех изменений одной операцией
    UnitOfWork.get_current().commit()  # через текущий поток
except Exception as e:
    print(e.args)
finally:
    UnitOfWork.set_current(None)  # закрываем текущий поток
